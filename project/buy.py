...
class Buy():
    ...
    @classmethod
    def get_buy_customer_last_price(cls, model: int, customer_id: int) -> None:
    ...
    @classmethod
    def get_buy_retailer_current_price(cls, dt, model: int):
        ...
        current_price_val = r().get_retailer_current_price(dt=dt,
                                                           ...
                                                           favorite_kind_cheese=cls.customer_data_dict.get(
                                                               'favorite_kind_cheese'))
        cls.retailer_current_price = current_price_val

    @classmethod
    def get_retailer_cheaper_kind_cheese(cls, dt, model: int) -> None:
    ...
    @classmethod
    def get_retailer_min_price(cls, dt, model: int) -> None:
        ...
        return r().get_retailer_min_price(dt=dt,
                                          ...
                                          retailer_top_list=m.RETAILER_TOP)

    @classmethod
    def make_decision_purchase_product(cls, dt, model: int, customer_id: int) -> None:
        ...
        cls.get_buy_customer_last_price(model=model, customer_id=customer_id)
        cls.get_buy_retailer_current_price(dt=dt, model=model)

        # 1
        # Определим текущий баланс продукта у покупателя
        cls.balance_cheese = None
        delta = cls.customer_data_dict.get('current_value_cheese') - cls.customer_data_dict.get(
        ...
        for _ in range(1):
            ...
            cls.retailer_current_min_price_retailer, cls.retailer_current_min_price_val = \
                cls.get_retailer_min_price(dt=dt, model=model)
        ...
        if cls.number_option in [2]:
            ...
            cls.customer_buy_dict['price'] = cls.get_retailer_cheaper_kind_cheese(dt=dt, model=model)
            cls.customer_buy_dict['quantity'] = random.choice([_ for _ in m.BUY_VALUE_CHEESE if _ <= 500])
            cls.balance_cheese = cls.balance_cheese + cls.customer_buy_dict.get('quantity')
    ...
    @classmethod
    def process_purchase(cls, dt, model: int, customer_id: int, script=script_add_table_sales_record) -> None:
